...
void UInworldCharacterPlaybackA2F_2::BeginPlay_Implementation()
{
	Super::BeginPlay_Implementation();

	AudioComponent = Cast<UAudioComponent>(OwnerActor->GetComponentByClass(UAudioComponent::StaticClass()));

	if (ensureMsgf(AudioComponent.IsValid(), TEXT("UInworldCharacterPlaybackA2F_2 owner doesn't contain AudioComponent")))
	{
		...
		SoundStreaming->OnSoundWaveProceduralUnderflow = FOnSoundWaveProceduralUnderflow::CreateUObject(this, &UInworldCharacterPlaybackA2F_2::GenerateData);

		AudioComponent->SetSound(SoundStreaming);
	}
}

void UInworldCharacterPlaybackA2F_2::EndPlay_Implementation()
{
...
void UInworldCharacterPlaybackA2F_2::Tick_Implementation(float DeltaTime)
{
...
void UInworldCharacterPlaybackA2F_2::OnCharacterUtterance_Implementation(const FCharacterMessageUtterance& Message)
{

	if (Message.SoundData.Num() > 0 && Message.bAudioFinal)
	{
		A2FData = Message.A2FData;
		A2FDataUpdateHandle = A2FData->OnCharacterMessageUtteranceA2FDataUpdate.AddUObject(this, &UInworldCharacterPlaybackA2F_2::OnCharacterMessageUtteranceA2FDataUpdate);
...
void UInworldCharacterPlaybackA2F_2::OnCharacterUtteranceInterrupt_Implementation(const FCharacterMessageUtterance& Message)
{
	FScopeLock ScopeLock(&QueueLock);
...
void UInworldCharacterPlaybackA2F_2::OnCharacterMessageUtteranceA2FDataUpdate()
{
	FScopeLock ScopeLock(&QueueLock);
...
void UInworldCharacterPlaybackA2F_2::GenerateData(USoundWaveProcedural* InProceduralWave, int32 SamplesRequired)
{
	if (!bIsActive)
	{
		return;
